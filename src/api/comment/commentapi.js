...
export const getComments = async (id) => {
  const response = await axios.get( `${process.env.REACT_APP_SERVER}/api/posts/${id}/comments`); 
  return response.data.comments;
};

// 댓글 추가 기능
export const addComment = async ({id, newContent }) => {
  await axios.post(`${process.env.REACT_APP_SERVER}/api/posts/${id}/comments`, newContent);
};
